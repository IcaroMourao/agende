...
public class JWTUtils {
	...
	public static Authentication getAuthentication(HttpServletRequest request) throws AuthenticationFailedException {
		...
		if (token != null) {
			...
			if (isValidPayload(payload))
				return new UsernamePasswordAuthenticationToken(payload.getSubject(), null, Collections.emptyList());
		}
		return null;
	}
	...
	private static String generateToken(String username, long now, long expirationTime) {
		String token = Jwts.builder().setSubject(username).setExpiration(new Date(now + expirationTime))
	...
	private static JWTPayload getTokenPayload(String token) throws AuthenticationFailedException {
		...
		} catch (MalformedJwtException | SignatureException | PrematureJwtException | UnsupportedJwtException e) {
